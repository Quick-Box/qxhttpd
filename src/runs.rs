...
pub fn run_id_from_values(value_map: &QxValueMap) -> anyhow::Result<i64> {
    let QxValue::Number(run_id) = value_map.get("run_id").ok_or(anyhow!("Change must contain run_id key."))? else {
        return Err(anyhow!("run_id key must contain number."))
    };
    Ok(*run_id)
}

#[post("/api/event/<event_id>/changes/run-update-request", data = "<change>")]
pub async fn add_run_update_request_change(event_id: EventId, session_id: QxSessionId, change: Json<QxValueMap>, api_token: QxApiToken, state: &State<SharedQxState>, db: &State<DbPool>) -> Result<(), Custom<String>> {
    let user = user_info(session_id, state)?;
    let value_map = change.into_inner();
    let run_id = run_id_from_values(&value_map).map_err(anyhow_to_custom_error)?;
    let data_type = DataType::RunUpdateRequest;
    let data = ChangeData::RunUpdateRequest(value_map.clone());
    add_change(event_id, "www", data_type, data, Some(run_id), Some(user.email.as_str()), state).await.map_err(anyhow_to_custom_error)?;
    if let Err(e) = state.read().expect("not poisoned")
        .broadcast_runs_change((event_id, value_map)) {
...
async fn add_run_updated_change(change: Json<QxValueMap>, api_token: QxApiToken, state: &State<SharedQxState>, db: &State<DbPool>) -> Result<(), Custom<String>> {
    let event = load_event_info_for_api_token(&api_token, db).await?;
    let value_map = change.into_inner();
    let run_id = run_id_from_values(&value_map).map_err(anyhow_to_custom_error)?;
    let data_type = DataType::RunUpdated;
    let data = ChangeData::RunUpdated(value_map);
